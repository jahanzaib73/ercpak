@extends('layouts.app')
@section('country-active-class', 'active')

@section('content')
    <div class="container-fluid">
        <div class="page-head">
            <h4 class="mt-2 mb-2">Countries</h4>
                    ...
                    <div class="card m-b-30">
                        <div class="card-body">
                            ...
                            <form role="form" method="POST"
                                action="{{ isset($data['country']) ? route('countries.update', ['id' => $data['country']->id]) : route('countries.store') }}">
                                @csrf
                                <div class="row">
                                    <div class="col-md-5">
                                        <div class="form-group">
                                            ...
                                            <input type="text"
                                                value="{{ old('name', isset($data['country']) ? $data['country']->name : '') }}"
                                    ...
                                    <div class="col-md-5">
                                        <div class="form-group">
                                            ...
                                            <select name="status" id="status" class="form-control ml-2">
                                                <option value="1"
                                                    {{ isset($data['country']) && $data['country']->status == 1 ? 'selected' : '' }}>
                                                    Active</option>
                                                <option value="0"
                                                    {{ isset($data['country']) && $data['country']->status == 0 ? 'selected' : '' }}>
                                                    In-active</option>
                                            </select>
                                            @error('status')
                                                <span class="error">
                            ...
                            <div class="col-sm-12">
                                <div class="table-responsive">
                                    <table id="datatable" class="table table-hover m-b-0">
                                        ...
                                        <tbody>
                                            @foreach ($data['countries'] as $type)
                                                <tr>
                                                    ...
                                                    <td>
                                                        @if (Auth::user()->can('Edit Country'))
                                                            <a class="btn btn bg-info btn-sm edit text-white_record text-white"
                                                                href="{{ route('countries.index', ['id' => $type->id]) }}"><i
                                                        ...
                                                        @if (Auth::user()->can('Delete Country'))
                                                            ...
                                                            <a class="btn btn-danger btn-sm"
                                                                ...
                                                                href="{{ route('countries.delete', ['id' => $type->id]) }}"><i
